...
namespace KangHui.JianHui.Utils.WebControls
{
    ...
    /// 用脚本实现的日历控件
    /// </summary>
    [Description("用脚本实现的日历控件")]
    [Designer(typeof(ClientCalendarDesigner))]
    [ToolboxData("<{0}:ClientCalendar runat=\"server\"></{0}:ClientCalendar>")]
    public class ClientCalendar : TextBox
    {
        bool write;
        int defaultYear = DateTime.Now.Year;
        int defaultMonth = DateTime.Now.Month;
        int beforeDefaultYearCount = 10;
        int afterDefaultYearCount = 20;

        private const string SCRIPTKEY = "ClientCalendarScript";
        ...
        [Category("自定义属性")]
        [DefaultValue(10)]
        [Description("默认选择年份之前的年数")]
        public int BeforeDefaultYearCount
        {
            get
            {
                return beforeDefaultYearCount;
            }
            set
            {
                beforeDefaultYearCount = value;
            }
        }
        ...
        [Category("自定义属性")]
        [DefaultValue(20)]
        [Description("默认选择年份之后的年数")]
        public int AfterDefaultYearCount
        {
            get
            {
                return afterDefaultYearCount;
            }
            set
            {
                afterDefaultYearCount = value;
            }
        }
        #endregion
		...
		var _VersionInfo=""Version:2.1&#13;作者:李秋楠&#13;MAIL: liqn@bitauto.com""	//版本信息

		//==================================================== WEB 页面显示部分 =====================================================
		var strFrame;		//存放日历层的HTML代码
		document.writeln('<iframe bgcolor=""#000000"" id=meizzDateLayer name=meizzDateLayer Author=wayx frameborder=0 style=""position: absolute;  width: 186; height: 247; z-index: 9998; display: none""></iframe>');
		strFrame='<style>';
		...
		strFrame+='        <td colspan=3 align=right Author=meizz><span onclick=parent.clearDate() style=""font-size:12px;cursor: hand""';
		strFrame+='         Author=meizz ><u>清除</u></span>&nbsp;<span onclick=parent.closeLayer() style=""font-size:12px;cursor: hand""';
		strFrame+='         Author=meizz ><u>关闭</u></span>&nbsp;</td></tr>';
		...
		strFrame+='             Author=meizz align=center><input Author=meizz style=""cursor:hand""  type=button class=button value=""今天"" onclick=""parent.meizzToday()"" ';
		...
		meizzDateLayer.document.writeln(strFrame);
		meizzDateLayer.document.close();		//解决ie进度条不结束的问题

		//==================================================== WEB 页面显示部分 ======================================================
		var outObject;
		var outButton;		//点击的按钮
		var outDate="""";		//存放对象的日期
		var odatelayer=meizzDateLayer.document;		//存放日历对象
		function setday(tt,obj) //主调函数
		{
			...
			if(r!=null){
				...
				outDate="""";
				meizzSetDay(" + DefaultYear + @", " + DefaultMonth + @");  //加入自定义默认选择年月的功能
			}
			dads.display = '';
		...
		function tmpSelectYearInnerHTML(strYear) //年份的下拉框
		{
		    ...
		    var m = (strYear) ? strYear : " + DefaultYear + @";
		    if (m < 1000 || m > 9999) {alert(""年份值不在 1000 到 9999 之间！"");return;}
		    var n = parseInt(m) - " + BeforeDefaultYearCount + @";   //加入自定义默认选择年前后范围的功能
            var o = parseInt(m) + " + AfterDefaultYearCount + @";   
		    if (n < 1000) {n = 1000; o = 1000 + " + BeforeDefaultYearCount + @" + " + AfterDefaultYearCount + @";}
		    if (o > 9999) {n = 9999 - " + BeforeDefaultYearCount + @" - " + AfterDefaultYearCount + @"; o = 9999;}
		    ...
		    var m = (strMonth) ? strMonth : " + DefaultMonth + @";
		    ...
		    for (i = 0; i < 39; i++)
		    {   
                //var elementMeizzDay = odatelayer.getElementById(""meizzDay"");
        ...
        ";
            ...
            {
                Page.ClientScript.RegisterStartupScript(Page.GetType(), SCRIPTKEY, strJs);
            }
        }
        #endregion

        #region 重写AddAttributesToRender方法
        /// <summary>
